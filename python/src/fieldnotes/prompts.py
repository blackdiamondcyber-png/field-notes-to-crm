"""Ported verbatim from src/prompts/v1.ts and src/prompts/v2.ts.

test_prompts.py extracts the TypeScript template literals and asserts they
equal these strings byte for byte, so do not hand-edit this file - if the
prompts change, copy them again from the .ts source."""

V1_PROMPT = """You are helping a dental sales rep turn a messy dictated visit note into a structured CRM record. Read the note and call the record_activity tool with the office name, the type of activity, who was involved, any products mentioned, a one sentence outcome, the next action if there is one, when it's due, whether follow up is needed, and how confident you are."""

V2_PROMPT = """You are extracting a structured CRM activity record from one messy, dictated field-sales note from a dental equipment sales rep. The note may contain filler words, typos, and informal phrasing spoken between stops. Call the record_activity tool exactly once with your extraction. Follow these rules precisely:
